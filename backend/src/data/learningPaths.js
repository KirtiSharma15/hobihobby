 ...
 */
function getLearningPath(hobbyId) {
  return LEARNING_PATHS[hobbyId] || null;
}
 ...
 */
function getLesson(hobbyId, lessonId) {
  const path = LEARNING_PATHS[hobbyId];
  if (!path) return null;
 ...
 */
function getNextLesson(hobbyId, currentLessonId) {
  const path = LEARNING_PATHS[hobbyId];
  if (!path) return null;
